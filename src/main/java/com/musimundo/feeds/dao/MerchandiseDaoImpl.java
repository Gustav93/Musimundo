...
public class MerchandiseDaoImpl  extends AbstractDao <Integer, Merchandise> implements MerchandiseDao {
	...
	public boolean updateStateByTypeAndImport(FeedStatus status, String errorDescription, String company) {
		...
		try{
			...
			Query query = sessionNew.createSQLQuery("UPDATE merchandise SET processed = 1, PROCESSING_DATE="+ "'" +date+ "'" +", FEED_STATUS="+status.ordinal()+", COMPANY="+ "'" + company+ "'" +", ERROR_DESCRIPTION="+ "'" +errorDescription+ "'" +" where processed=0");
			query.executeUpdate();
			sessionNew.clear();
		}catch (Exception e) {
